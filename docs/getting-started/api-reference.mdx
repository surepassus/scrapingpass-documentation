...
|      [`scrape_google`](#scrape_google)      | Boolean |                      Must be specified true when remote host is Google.                      |          false         |          false          |    -    |
...
|               [`html`](#html)               | Boolean |                     Explicitly specify to return HTML of remote response.                    |          true          |           true          |    -    |
|       [`js_rendering`](#js_rendering)       | Boolean |             Use JS Rendering rather than normal requests using headless browsers.            |          false         |          false          |    -    |
|         [`screenshot`](#screenshot)         | Boolean |                      Use JS Rendering and render screenshot of the page.                     |          false         |          false          |    -    |
|                [`pdf`](#pdf)                | Boolean |                         Use JS Rendering and render PDF of the page.                         |          false         |          false          |    -    |
...
|      [`google_search`](#google_search)      | Boolean |                         Explicitly specify to use Google Search mode.                        |          false         |          false          |    -    |
...
### `forward_headers`
Determines whether to forward the headers along with request to remote. In REST API mode, the headers which are needed to be forwarded must be prefixed with `Sp-`. For example, if you need to pass a Referer header, you can do so by sending header with key `Sp-Referer` and specifying `forward_headers=true`. Scrapingpass will strip Sp- and forward your Referer header normally.
